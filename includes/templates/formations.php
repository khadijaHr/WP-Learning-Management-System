<?php
    ...
    // Ajouter la condition WHERE seulement pour les Coache
    if($user->role === 'Coach'):
        $query .= " WHERE f.statut = 1";
    endif;

    if($user->role === 'Participant'):
        $query .= " WHERE fich.visible_participants = 1";
    endif;
    
    $query .= " GROUP BY f.id";

    if($user->role === 'Master Coach'):    
        $query .= " ORDER BY f.created_at DESC";
    endif;

    // Exécuter la requête
    $formations = $wpdb->get_results($query);
    ...
    <div class="grid">
        <?php foreach ($formations as $formation): ?>
            ...
            <div class="middle">                      
                ...
                <?php if ($user->role === 'Master Coach'):
                    $active_class = $formation->statut ? 'btn-active' : 'btn-inactive';
                    $nonce = wp_create_nonce('toggle_formation_' . $formation->id);
                ?>
                    <a href="#" class="btn_edit toggle-formation <?= $active_class ?>"
                    data-formation-id="<?= esc_attr($formation->id) ?>"
                    data-nonce="<?= esc_attr($nonce) ?>">
                        <?= $formation->statut ? '' : 'Activé' ?>
                    </a>
                <?php endif; ?>
            </div>
            <div class="bottom">
...
<script>
    ...
    // Modifiez la partie AJAX pour envoyer correctement les fichiers
    $('#formation-form').submit(function(e) {
        ...
        // Ajouter les fichiers pour chaque jour
        $('.jour-item').each(function(index) {
            const fileInput = $(this).find('input[type="file"]')[0];
            if (fileInput.files.length > 0) {
                $.each(fileInput.files, function(i, file) {
                    formData.append(`jours[${index+1}][fichiers][]`, file);
                });
            }
        });
...
<script>
jQuery(document).ready(function ($) {
    $(document).on('click', '.toggle-formation', function (e) {
        e.preventDefault();
        var button = $(this);
        var formationId = button.data('formation-id');
        ...
        $.ajax({
            ...
            dataType: 'json',
            data: {
                action: 'toggle_formation_status',
            ...
            success: function (response) {
                if (response.success) {
                    button.toggleClass('active')
                          .text(response.data.new_status ? 'Désactiver' : 'Activé');
                } else {
                    alert(response.data?.message || 'Erreur inconnue (serveur)');
